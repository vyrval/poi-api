...
import { linkEventsToPOI, linkEventsToPOI2 } from '../src/utils.js';
...
describe('linkEventsToPOI2', () => {
  ...
  it('should respect output format', () => {
    ...
    const res = linkEventsToPOI2(CLUSTERS, INPUT);
    assert.deepEqual(res, expected);
  });
  it('should return even if no data', () => {
    const expected = {
    ...
    const res = linkEventsToPOI2({}, INPUT);
    assert.deepEqual(res, expected);
  });
  it('should return an empty object if no input', () => {
    const res = linkEventsToPOI2({}, []);
    assert.deepEqual(res, {});
  });
  it('should return an empty object if no input', () => {
    const res = linkEventsToPOI2(CLUSTERS, []);
    assert.deepEqual(res, {});
  });
});
